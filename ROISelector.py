...
class ROISelector:
    def __init__(self, image_path):
        ...
        self.image = cv2.resize(self.original_image, (512, 512))  # Resize for ROI selection
    ...
    def draw_rectangle(self, event, x, y, flags, param):
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            if self.drawing:
                self.image = cv2.resize(self.original_image, (512, 512))  # Reset image to original size each time
                cv2.rectangle(self.image, self.rect_start, (x, y), (0, 255, 0), 2)
        ...
        while True:
            ...
            if key == 13:  # Enter key
                if self.rect_start and self.rect_end:
                    ...
                    x1_orig = int(x1 * self.original_width / self.resized_width)
                    y1_orig = int(y1 * self.original_height / self.resized_height)
                    x2_orig = int(x2 * self.original_width / self.resized_width)
                    y2_orig = int(y2 * self.original_height / self.resized_height)
                    ...
                    cropped_image = self.original_image[y1_orig:y2_orig, x1_orig:x2_orig]
